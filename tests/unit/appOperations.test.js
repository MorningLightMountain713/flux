...
describe('appOperations tests', () => {
  ...
  describe('reconcileInstalledApps named-placement tests', () => {
    // Named placement is declarative: the sweep removes an installed app from a
    // node the current global spec no longer names, and never applies the
    // count-based over-instance eviction to named mode (it sheds the newest
    // instance, which can be a still-targeted replica mid-transition).
    ...
    // The registry spec carries a REAL Placement instance so the sweep runs the
    // actual mode/resolution machinery rather than a hand-mocked shadow of it.
    async function registrySpecWith(placementBlob, { hash = 'h1' } = {}) {
      const { Placement } = await import('@runonflux/flux-spec');
      return {
        name: 'myapp', hash, isEncrypted: false, placement: Placement.from(placementBlob),
      };
    }

    function setup(registrySpec, { installedHash = 'h1', locations = [] } = {}) {
      // eslint-disable-next-line global-require
      const fluxNetworkHelper = require('../../ZelBack/src/services/fluxNetworkHelper');
      // eslint-disable-next-line global-require
      ...
      sinon.stub(appsRepository, 'listInstalledApps').resolves([
        { name: 'myapp', hash: installedHash, spec: { instances: 1 } },
      ]);
      sinon.stub(appsRepository, 'getGlobalAppInfo').resolves(registrySpec);
      // Null deployments make reconcileApp a no-op, isolating the removal branching.
      sinon.stub(deploymentProvider, 'getInstalledDeployment').resolves(null);
      sinon.stub(deploymentProvider, 'buildDeployment').resolves(null);
      appLocationStub = sinon.stub(registryManager, 'appLocation').resolves(locations);
    ...
    it('removes the app when named placement does not target this node', async () => {
      const registrySpec = await registrySpecWith({ targetIps: { '9.9.9.9': ['s1'] } });
      setup(registrySpec);

      await appOperations.reconcileInstalledApps();

      expect(uninstallStub.calledOnceWith('myapp', sinon.match({ broadcastRemoval: true })), 'de-targeted named app removed with broadcast').to.be.true;
    });

    it('keeps the app and skips over-instance eviction when this node holds a named replica', async () => {
      const registrySpec = await registrySpecWith({ targetIps: { '44.55.66.77': ['s1'] } });
      // Five reported locations against derived instances 1 would trip the
      // count-based eviction if it (wrongly) applied to named mode.
      setup(registrySpec, {
        locations: [
          { ip: LOCAL_IP, runningSince: 5 },
          { ip: '1.1.1.1:16127', runningSince: 1 },
          { ip: '2.2.2.2:16127', runningSince: 2 },
          { ip: '3.3.3.3:16127', runningSince: 3 },
          { ip: '4.4.4.4:16127', runningSince: 4 },
        ],
      });

      await appOperations.reconcileInstalledApps();

      expect(uninstallStub.called, 'targeted named replica must not be evicted').to.be.false;
      expect(appLocationStub.called, 'named mode never consults the location count').to.be.false;
    });

    it('leaves the app untouched when a co-located union fails the resolution loud', async () => {
      // One physical node addressed through both identity forms: resolution
      // throws, the per-app catch logs it, and nothing is destroyed.
      const registrySpec = await registrySpecWith({
      ...
      expect(uninstallStub.called, 'a failed resolution must not remove anything').to.be.false;
    });

    it('still applies over-instance eviction to loose placement when this node is newest', async () => {
      const registrySpec = await registrySpecWith({ targetIps: { '44.55.66.77': null, '9.9.9.9': null } });
      setup(registrySpec, {
        locations: [
      ...
      expect(uninstallStub.calledOnceWith('myapp', sinon.match({ broadcastRemoval: true })), 'newest surplus loose instance evicted').to.be.true;
    });
  });
