...
// The schema the dispatcher resolves once and hands to each handler.
const LABEL_KEYS = { IDENTIFIER: 'io.runonflux.identifier', PLAYGROUND_SESSION: 'io.runonflux.playground' };
...
describe('containerEventBridge', () => {
  ...
  describe('die', () => {
    it('enqueues a reconcile for a flux container crash die, keyed by the bare identifier', async () => {
    ...
    it('reads the identity label rather than stripping the name', async () => {
      const event = {
        Action: 'die',
    ...
    it('keys a component named flux* by its own identifier, not one four characters short', async () => {
      // `fluxproxy` is a legitimate component name, so its container is
      // `fluxfluxproxy_app` and its identifier `fluxproxy_app`. A strip that
      // sniffed the identifier instead of the name yielded `proxy_app` — a
      // DIFFERENT component, whose containers and durable operator intent the
      // reconciler would then act on.
      const event = {
        Action: 'die',
  ...
  describe('start', () => {
    it('wakes the dependents of a started container (satisfies a dependsOn started)', async () => {
      containerEventBridge.handleContainerStart(startEvent('fluxdb_app'), LABEL_KEYS);
    ...
    it('ignores non-flux containers', async () => {
      containerEventBridge.handleContainerStart(startEvent('postgres'), LABEL_KEYS);
      expect(stubs.appReconciler.enqueueDependents.called).to.be.false;
    });
  });
