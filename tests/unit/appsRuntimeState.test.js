...
describe('appsRuntimeState tests', () => {
  ...
  describe('prepareCollection (merge-dedupe + unique index)', () => {
    ...
    it('merges twins field-wise: lock + start/proven flags are OR, histories union, newest exit wins', async () => {
      docs = [
        {
          identifier: 'www_App', operatorStopped: true, restartHistory: [100, 200], hasEverStarted: true, updatedAt: 1000,
        },
        {
      ...
      expect(merged.hasEverStarted).to.equal(true); // started on either twin = started here (gates firstStart)
      expect(merged.hasSuccessfullyStarted).to.equal(true); // proven on either twin = proven here (ends the trial)
