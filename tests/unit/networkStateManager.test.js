...
describe('networkStateManager tests', () => {
  ...
  describe('getRandomSocketAddressSample', () => {
    ...
    it('returns one address per distinct /16 when distinctPrefixes is set', async () => {
      ...
      const res = await nsm.getRandomSocketAddressSample(3, { distinctPrefixes: true });
    ...
    it('returns at most count addresses', async () => {
      expect(await nsm.getRandomSocketAddressSample(1, { distinctPrefixes: true })).to.have.lengthOf(1);
    });

    it('returns an empty array for count 0', async () => {
      expect(await nsm.getRandomSocketAddressSample(0)).to.deep.equal([]);
    });
