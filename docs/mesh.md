...
7. [Gossip vs consensus — what your software needs](#gossip-vs-consensus--what-your-software-needs)
8. [Rules that keep your cluster safe](#rules-that-keep-your-cluster-safe)
...
actor (`rs.initiate`, cluster creation), gate it on `FLUX_MESH_ORDINAL = 0`.
Exactly one member answers to it.
...
| Bootstrap | point at the seed name | ordinal 0 initiates; others join |
...
- **Quorum loss is yours to recover.** If a majority of a consensus cluster
  ...
  mesh, not Kubernetes) can conjure a quorum back.
