...
const { extractIp, extractPort, socketAddressesMatch, DEFAULT_API_PORT } = require('../utils/socketAddressUtils');
          ...
          if (localSocketAddr) {
            ...
            if ((!ip)) {
              ...
              if (!runningAppsNames.includes(identifier)) {
                ...
                if (index === 0 && !activePrimaryByIdentifier.has(identifier)) {
                ...
                } else if (!scheduledPrimaryStart.has(identifier) && activePrimaryByIdentifier.has(identifier) && !socketAddressesMatch(activePrimaryByIdentifier.get(identifier), localSocketAddr)) {
                  ...
                  if (index === 0) {
                    ...
                    const previousMasterIndex = runningAppList.findIndex((x) => socketAddressesMatch(x.ip, activePrimaryByIdentifier.get(identifier)));
              ...
              if (!socketAddressesMatch(localSocketAddr, ip) && runningAppsNames.includes(identifier)) {
                ...
                log.info(`activeStandby: stopping docker app:${appName} it's running on ip:${ip} and localSocketAddr is: ${localSocketAddr}`);
