 ...
 * - `static_host_map` carries each peer's container addresses, not only its
 *   block base — the shared overlay prefix makes nebula dial a container address
 *   directly, so its endpoint must be resolvable here.
 *
 * @param {{instance: string, appUuid: string, outpoint: string, listenPort: number,
 *   members: Array<{outpoint: string, address: string, block: string, endpoint: string, caShas: string[]}>,
 *   componentSlots: number[], sshClientPublicKey: string}} app
 *   members are the accepted peers, this node excluded; componentSlots are this
 *   app's component slots (for each peer's container addresses); sshClientPublicKey
 *   is the node-wide OpenSSH public key line the detector authenticates with
 * @returns {string} config.yml text
 */
function nebulaConfig({
  instance, appUuid, outpoint, listenPort, members, componentSlots, sshClientPublicKey,
}) {
  ...
  if (!Array.isArray(componentSlots) || componentSlots.some((s) => !Number.isInteger(s))) {
    throw new TypeError('componentSlots must be an array of integer slots');
  }
  ...
  // static_host_map carries each peer's endpoint keyed by every address that
  // resolves to it — its block base and each component container address. A
  // container address sits inside the shared overlay prefix, so nebula dials it
  // directly and needs its endpoint here; the peer answers with the block-base
  // identity and one tunnel serves them all. Sorted for a stable, golden output.
  const hostMapEntries = peers.flatMap((m) => [
    m.address,
    ...componentSlots.map((slot) => meshDerivation.memberAddress(appUuid, m.outpoint, slot)),
  ].map((addr) => ({ addr, endpoint: m.endpoint })))
  ...
  const lines = [
    ...
    'static_host_map:',
    ...hostMapEntries.map((e) => `  ${quoted(e.addr)}: [${quoted(e.endpoint)}]`),
    'lighthouse:',
    '  am_lighthouse: false',
    '  hosts: []',
    '  local_allow_list:',
