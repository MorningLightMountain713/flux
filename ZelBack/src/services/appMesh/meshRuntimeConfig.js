 ...
 * - `static_host_map` carries each peer's block base keyed to its endpoint; the
 *   base is the peer's nebula identity, and `unsafe_routes` route a peer's
 *   container block to it.
 * - `unsafe_routes` sends each peer's block via that peer's base address. A
 *   container address is outside this node's own certificate networks (its base
 *   only), so nebula routes it to the owning peer through the gateway rather than
 *   dialing it directly.
 *
