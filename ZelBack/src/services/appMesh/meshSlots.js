...
// converges because arbitration is a pure function of gossiped facts: the
// earliest runningSince wins, tiebroken on the outpoint — every observer
// reaches the same verdict, and the loser re-picks the next vacancy.
 ...
 *   2. the own installing claim's slot, kept while still vacant;
 *   3. a fresh pick of the lowest vacancy;
 *   4. null — every slot is held; this member is a standby.
 * Vacancy counts slots won by running members plus slots named in OTHER
 * nodes' standing install claims, so two concurrent installers split.
 *
...
async function appSlotView(appName, instances) {
  ...
  for (const claim of claims) {
    if (claim.ip !== localSocketAddr && Number.isInteger(claim.meshSlot)
      && claim.meshSlot >= 0 && claim.meshSlot < cap) {
      occupied.add(claim.meshSlot);
    }
  }

  // 2. The own installing claim's slot, while nothing else took it.
  const ownClaim = claims.find((claim) => claim.ip === localSocketAddr
    && Number.isInteger(claim.meshSlot) && claim.meshSlot >= 0 && claim.meshSlot < cap);
  if (ownClaim && !occupied.has(ownClaim.meshSlot)) {
