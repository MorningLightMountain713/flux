...
// is nonce || tag || ciphertext, and FluxOS owns both ends of the framing.
...
/**
 * AES-256-GCM open of a nonce || tag || ciphertext frame. Throws on auth failure
 * (wrong key, tampered ciphertext, or tampered aad) — never returns garbage.
 * @param {Buffer} key 32-byte key
 * @param {Buffer} framed nonce || tag || ciphertext
 * @param {Buffer} [aad]
...
function aeadDecrypt(key, framed, aad) {
  ...
  const tag = framed.subarray(NONCE_BYTES, NONCE_BYTES + TAG_BYTES);
  const ciphertext = framed.subarray(NONCE_BYTES + TAG_BYTES);
