...
async function reconcile(rawIdentifier) {
  ...
  notifyContainerStarted(identifier);
  // An unproven run must prove itself before its converge can settle: arm the
  // proof pass (the running branch latches on probe-healthy or uptime). The armed
  // retry is what holds onSettled open through the window.
  if (!proven) scheduleRetry(identifier, FIRST_RUN_PROOF_MS);
}
