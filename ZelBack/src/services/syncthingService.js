...
async function startSyncthing() {
  try {
    ...
    if (myDevice.status === 'error') {
      ...
      const exec = 'sudo nohup syncthing -logfile $HOME/.config/syncthing/syncthing.log --logflags=3 --log-max-old-files=2 '
        + '--log-max-size=26214400 --allow-newer-config --no-browser --home=$HOME/.config/syncthing &';
