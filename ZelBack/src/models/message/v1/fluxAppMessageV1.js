...
// import these as part of typedef
const { FluxAppSpecV8 } = require("../../spec/v8/fluxAppSpecV8");
const { FluxAppSpecV7 } = require("../../spec/v7/fluxAppSpecV7");
...
class FluxAppMessageV1 extends FluxAppMessageBase {
  ...
  static propValidators = {
    ...
    version: (input) =>
      this.validateNumber((input, { minValue: 1, maxDecimals: 0 })),
    ...
    ownerSignature: (input) =>
      this.validateString(input, {
        patterns: [/^[0-9a-zA-Z\/\+=]{88}$/, /^0x[0-9a-fA-F]{130}/],
      }),
    txId: (input) =>
      this.validateString(input, { patterns: [/^[0-9a-fA-f]{64}$/] }),
